...
1. ~~**Drop `content_original` + 4 bảng `*_bak*`**~~ → **ĐÓNG ISSUE 2026-09-12 (plan P5.1)**: backup đã có (2×`.bak` + restore drill PASS), nhưng quyết định **GIỮ `content_original` vĩnh viễn** — backfill không đọc cột này, 2 reader còn lại flag-off, 69MB/230MB ≈ 0 chi phí so với rủi ro mất bản gốc có thể re-scrape được. Xem AGENTS.md Boundaries.
...
2. ~~**Đổi `ddl-auto=update` → `validate`**~~ → **P4 2026-09-12: đã chạy diagnostic, PASS 0 issues** — boot-drill một lần `SPRING_JPA_HIBERNATE_DDL_AUTO=validate` qua env override (không đổi `application.properties`): `Started in 14.5s`, 0 ERROR/0 schema-warning (`tasks/evidence/p4-validate-drill.log`). Entity ↔ DB đang khớp; giữ `update` cho local theo đúng assumption "không deploy profile", drill lại khi cần.
3. **Timezone**: thống nhất 1 hệ (khuyến nghị: mọi `LocalDateTime.now()` trong writes → `Instant.now()` + cột `datetime2`, hoặc set cả container + SQL về cùng TZ); trước mắt **không dùng test "future date" làm pass/fail** cho tới khi sửa (đã biết systematic false-positive). → **P2 2026-09-12: ĐÃ CHỐT theo hướng ngược lại — KHÔNG migrate UTC**, thay bằng convention "mọi cột datetime2 = naive giờ VN, 1 nguồn ghi duy nhất" đã verify + ghi AGENTS.md (0 default constraint dùng SYSDATETIME/GETDATE; di chuyển chỉ tạo rủi ro khi chưa có consumer thứ hai).
4. ~~**DEFAULT_USER_PASSWORD / DEFAULT_ADMIN_PASSWORD**~~ → **P5.2 2026-09-12: đã doc vào AGENTS.md Boundaries** (verify `DatabaseSeeder:91-98` đọc 2 env này, default `password123` + WARN F58). Không code thêm.
