...
- [x] T8.5 Full UI functional gate — mọi trang chính (home/lessons/videos/decks 6 mode/speaking/search/profile/leaderboard/premium) + admin (dashboard/lessons/exercises/speaking-submissions/videos/video-attempts/users + CRUD create-lesson) qua UI thật dev :5173: **0 console error/warning**. Phát hiện & sửa 2 lỗi data: correct_answer sai của 755960/755963 (lesson 41881). Ghi chú: speaking mic cần user-gesture (headless từ chối → fail-soft message OK); CORS :4173 là dev-only config (prod same-origin). Backend suite 225/225, frontend 73/73, sweep 67/67.
- [x] T8.6 Cập nhật spec.md DoD + REPORT §4e/§4f + commit cuối
