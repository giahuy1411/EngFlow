/**
 * audit-v13-full Phase 3 — focused verification probe.
 *
...
(async () => {
  ...
  // ── 2. AdminLessonBuilder contrast (admin) ──────────────────────────────────
  {
    H.flushLimits(true);
    const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 } });
    const page = await ctx.newPage();
    await H.seedAuth(page, adm);
    await page.goto(H.APP + "/admin/445/build", { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(3000);
    const whiteOnAccent = await page.evaluate(`(${measureByText})('B', 'span')`);
    const white40 = await page.evaluate(`(${measureBySelector})('.text-white\\\\/40')`);
    const fg40 = await page.evaluate(`(${measureBySelector})('.text-foreground\\\\/40')`);
    const badge = await page.evaluate(`(${measureByText})('A', 'span')`);
    out.builder = { logoB: whiteOnAccent, closeWhite40: white40, closeFg40: fg40, badgeA: badge };
    console.log("builder logoB=" + JSON.stringify(whiteOnAccent));
    console.log("builder text-white/40=" + JSON.stringify(white40));
    console.log("builder text-foreground/40=" + JSON.stringify(fg40));
    await page.screenshot({ path: path.join(__dirname, "..", "..", ".specify", "specs", "audit-v13-full", "evidence", "shots", "v13-admin-builder-contrast.png") });
    await ctx.close();
  }
  ...
  await browser.close();
  fs.writeFileSync(path.join(__dirname, "..", "..", ".specify", "specs", "audit-v13-full", "evidence", "focused-probe.json"), JSON.stringify(out, null, 2));
