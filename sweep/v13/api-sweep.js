/**
 * audit-v13-full Phase 2 — API sweep over the live container (HTTP only, no browser).
 *
 ...
 *
 * Run: node sweep/v13/api-sweep.js
 * Out: .specify/specs/audit-v13-full/evidence/api-sweep.json
 */
...
(async () => {
  console.log("=== audit-v13-full API sweep ===");
  ...
  {
    const nm = "AUDIT-V13-API-" + PUBLIC_DECK;
    ...
    const lt = "AUDIT-V13-LESSON-" + PUBLIC_DECK;
    ...
    // deck". Both are now created here and removed by the `AUDIT-V13-API-%` cleanup below.
    const ownDeck = await req("POST", "/api/decks", { token: userToken, body: { name: "AUDIT-V13-API-F147", description: "audit f147 own", isPublic: false } });
    const ownDeckId = ownDeck.data?.id ?? ownDeck.data?.deckId ?? ownDeck.data?.data?.id;
    const foreignDeck = await req("POST", "/api/decks", { token: adminToken, body: { name: "AUDIT-V13-API-FOREIGN", description: "audit f147 foreign", isPublic: false } });
...
`;
    ...
    let out = "";
    try { out = execFileSync("python", ["sweep/v8/sqlrun.py", "sweep/v13/_cleanup.sql"], { encoding: "utf8", cwd: path.join(__dirname, "..", "..") }); }
  ...
  const OUT = path.join(__dirname, "..", "..", ".specify", "specs", "audit-v13-full", "evidence", "api-sweep.json");
