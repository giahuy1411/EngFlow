/**
 * audit-v13-full Phase 5 — performance probe, measured BEFORE any optimisation.
 *
 ...
 *
 * v13 delta vs v12: v12 measured 22 endpoints once. v13 re-measures all of them plus
 * more of the v1 API surface (streak current/history, auth/me, lesson structure/content,
 ...
 *
 * Run from repo root:  node sweep/v13/perf-probe.js
 * Out:                 sweep/v13/perf-before.json
 */
...
(async () => {
  ...
  const targets = [
    ...
    ["lesson structure",            "/api/lessons/41881/structure",               null,       "learner"],
    ...
    // --- v13 additions: rest of the v1 / gamification surface ---
    ["game session (memory)",       "/api/games/memory/10006",                    userToken,  "v13-new"],
    ["game session (mixed)",        "/api/games/mixed/10006",                     userToken,  "v13-new"],
    ["speaking prompts (admin)",    "/api/v1/admin/speaking-prompts?page=0&size=10", adminToken, "v13-new"],
    ["video attempts (my)",         "/api/v1/video-attempts",                     userToken,  "v13-new"],
    ["lesson submissions (my)",     "/api/lesson-submissions/my/lesson/41881/skill/SPEAKING", userToken, "v13-new"],
    ["video lessons (admin)",       "/api/v1/admin/video-lessons?page=0&size=10",  adminToken, "v13-new"],
    // --- admin surface ---
  ...
  const dir = path.join("sweep", "v13");
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, "perf-" + label + ".json"), JSON.stringify(out, null, 2));
  console.log("\nwrote sweep/v13/perf-" + label + ".json (" + results.length + " endpoints)");
})();
