/**
 * audit-v13-full Phase 2 — DEEP probes the base sweep does not cover.
 *
 ...
 *
 * Run: node sweep/v13/deep-probe.js
 * Out: .specify/specs/audit-v13-full/evidence/deep-probe.json
 */
...
(async () => {
  console.log("=== audit-v13-full DEEP probe ===");
  ...
  console.log("\n--- A. MULTIPLE_CHOICE guard (F-13-01) ---");
  const LESSON_TITLE = "AUDIT-V13-MC-" + PUBLIC_DECK;
  let lessonId = null;
  {
  ...
  const mc = (options, extra = {}) => ({
    lessonId, question: "AUDIT-V13-MC probe?", options: JSON.stringify(options),
  ...
  if (lessonId) {
    ...
    check("LISTENING with bare-letter options [\"A\",\"B\",\"C\",\"D\"] -> 400 (guard covers the same defect class)",
      listenBare.status === 400, `got ${listenBare.status} — LISTENING renders as a choice type too, but the guard is MULTIPLE_CHOICE-only`);
    ...
    check(`POST a copy of real row ${REAL_MC_ID}'s exact options -> 400 (admin cannot re-create it either)`,
      realCopy.status === 400, `got ${realCopy.status} — guard rejects the real row's legitimate content shape`);
  ...
  console.log("\n--- C. RESPONSE CONTRACT (field names + types) ---");
  {
    ...
    const nm = "AUDIT-V13-DEEP-" + PUBLIC_DECK;
  ...
  console.log("\n--- CLEANUP ---");
  {
    ...
    // SQL-level verification that no AUDIT-V13-MC lesson / AUDIT-V13-DEEP deck survived
    const sql = `
...
`;
    ...
    let out = "";
    try { out = execFileSync("python", ["sweep/v8/sqlrun.py", "sweep/v13/_deep-cleanup.sql"], { encoding: "utf8", cwd: path.join(__dirname, "..", "..") }); }
  ...
  const OUT = path.join(__dirname, "..", "..", ".specify", "specs", "audit-v13-full", "evidence", "deep-probe.json");
