/**
 * CLS probe (Playwright, PerformanceObserver + attribution).
 *
 ...
 *
 * Run from repo root:
 *   cmd /c "set NODE_PATH=%APPDATA%\npm\node_modules&& node sweep/harness/cls-probe.js"
 * Out: <evidence>/cls-before.json
 */
const fs = require("fs");
...
(async () => {
  ...
  await browser.close();
  // audit-v15 F-15-16: write beside THIS script via the shared config, not into sweep/v13.
  const EVID = require("./_config.js").OUT;
  fs.mkdirSync(EVID, { recursive: true });
  fs.writeFileSync(path.join(EVID, "cls-before.json"), JSON.stringify(out, null, 2));
  console.log("\nwrote " + path.join(EVID, "cls-before.json"));
})();
