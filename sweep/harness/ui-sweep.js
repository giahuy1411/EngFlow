/**
 * UI sweep (Playwright driver) — route×role matrix, a11y, contrast, responsive.
 *
 ...
 *
 * Run: node sweep/harness/ui-sweep.js [--audit <name>] [--out <dir>]
 */
...
const ROUTES = [
  ...
  { p: "/videos",                 expect: { anon: "/premium",      student: "/videos",       admin: "/videos" } },
  { p: `/videos/${V}`,            expect: { anon: "/premium",      student: `/videos/${V}`,  admin: `/videos/${V}` } },
  ...
  { p: "/speaking",               expect: { anon: "/premium",      student: "/speaking",     admin: "/speaking" } },
  { p: "/speaking/history",       expect: { anon: "/premium",      student: "/speaking/history", admin: "/speaking/history" } },
  { p: `/speaking/${P}`,          expect: { anon: "/premium",      student: `/speaking/${P}`, admin: `/speaking/${P}` } },
  { p: `/speaking/${P}/record`,   expect: { anon: "/premium",      student: `/speaking/${P}/record`, admin: `/speaking/${P}/record` } },
  ...
  // audit-v15 L3: /admin/:id/build was removed with the Lesson Builder (Đường B).
  // The catch-all route (/:pathMatch(.*)*) now redirects it to "/" for every role, so
  // this entry asserts the route is GONE — a regression guard, not a live route.
  { p: `/admin/${L}/build`,       expect: { anon: "/",             student: "/",             admin: "/" } },
...
(async () => {
  ...
  // audit-v15 hardening (L1-b class): the cleanup MUST run even if the walk throws
  // part-way. ROUTES includes /premium/checkout, whose component mints a REAL PENDING
  // payment row on mount, so a throw before cleanup would leave that row behind — the
  // exact defect this pass fixes. Everything from the login through the screenshot loop
  // is wrapped in try/finally; the finally always closes the browser and cleans up.
  let clean = { ok: false };
  let residue = false;
  let fatal = null;
  try {
  ...
  for (const rt of ROUTES) {
    for (const role of roles) {
      ...
      // audit-v14 P1 (kept in the committed harness): guards append ?redirect=<path>, so
      // compare the PATHNAME only — comparing the full URL to a bare path is a false failure.
      const landed = (page.url().replace(APP, "").split("?")[0]) || "/";
      const want = rt.expect[role].split("?")[0];
  ...
  for (const s of SHOTS) {
    ...
    // Record the failure but still fall through to cleanup + evidence write: a partial
    // sweep with its writes cleaned up is far better than a crashed one that leaks rows.
    fatal = e;
    console.error("WALK FAILED: " + (e && e.stack ? e.stack : e));
  } finally {
    await browser.close();

    // audit-v15 F-15-09 (fixed in the committed harness): ROUTES includes
    // /premium/checkout, whose component mints a REAL PENDING payment row on mount. This
    // sweep used to leave it behind (parity 12 -> 14) because it had no cleanup at all.
