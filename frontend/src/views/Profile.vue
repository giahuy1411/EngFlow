    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div class="bg-card border-2 border-foreground rounded-md p-8 shadow-pop-xl mb-8 relative overflow-hidden">
        ...
        <div class="flex flex-col md:flex-row items-center gap-8 relative z-10">
          ...
          <div class="flex gap-4">
            <div class="text-center"><p class="font-black text-2xl text-accent-ink">{{ currentStreak }}</p><p class="font-bold text-[10px] uppercase tracking-widest text-muted-foreground">Streak</p></div>
            <div class="text-center"><p class="font-black text-2xl text-secondary-ink">{{ user.totalPoints || 0 }}</p><p class="font-bold text-[10px] uppercase tracking-widest text-muted-foreground">Điểm</p></div>
      ...
      <section class="mb-8 border-2 border-foreground bg-card p-6 shadow-pop-xl" aria-labelledby="subscription-title">
        <div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <router-link v-if="!premiumStore.isPremium && !auth.isAdmin" to="/premium" class="inline-flex shrink-0 items-center justify-center rounded-full border-2 border-foreground bg-accent-strong px-5 py-3 font-black text-white shadow-pop transition-transform hover:-translate-y-0.5">Nâng cấp Premium</router-link>
      ...
      <div class="grid md:grid-cols-2 gap-6 mb-8">
        <div class="bg-card border-2 border-foreground rounded-md p-6 shadow-pop-lg hover:-translate-y-1 transition-all duration-300">
          <div class="flex items-center gap-3 mb-3"><div class="w-10 h-10 bg-accent/10 border-2 border-foreground rounded-full flex items-center justify-center"><BookOpen class="w-5 h-5 text-accent-ink" /></div><span class="font-bold text-xs uppercase tracking-wider text-muted-foreground">Bài học</span></div>
        ...
        <div class="bg-card border-2 border-foreground rounded-md p-6 shadow-pop-lg hover:-translate-y-1 transition-all duration-300"><div class="flex items-center gap-3 mb-3"><div class="w-10 h-10 bg-secondary/10 border-2 border-foreground rounded-full flex items-center justify-center"><Zap class="w-5 h-5 text-secondary-ink" /></div><span class="font-bold text-xs uppercase tracking-wider text-muted-foreground">Streak</span></div><p class="font-black text-3xl">{{ currentStreak }}</p></div>
      </div>
