  ...
  <div class="flashcard-game w-full max-w-6xl mx-auto px-2 sm:px-4 min-h-[calc(100vh-180px)] flex items-center">
    <div class="relative w-full aspect-video bg-gradient-to-br from-yellow-50 to-yellow-100 border-4 border-black rounded-2xl shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
      ...
      <div class="absolute top-0 left-0 right-0 flex justify-between items-center px-4 sm:px-6 py-3 z-10">
        ...
        <div class="font-black text-base sm:text-lg bg-yellow-300 border-2 border-black rounded-full px-4 py-1" v-if="words.length">
      ...
      <div v-else-if="!sessionComplete && words.length > 0" class="absolute inset-0 flex flex-col pt-14 px-4 sm:px-6 pb-4 sm:pb-6">
        ...
        <div v-if="!isFlipped" class="flex-none text-center">
          <p class="font-bold text-gray-500 uppercase tracking-widest text-xs sm:text-sm">Think of the answer, then click card to flip</p>
        </div>
        ...
        <div v-else class="flex-none animate-fade-in-up">
          ...
          <div class="grid grid-cols-4 gap-2 sm:gap-3">
            <button @click="rateWord(0)" class="py-2 sm:py-3 bg-red-400 text-black font-black uppercase border-4 border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 hover:shadow-[4px_6px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none transition-all text-xs sm:text-sm">
              Forgot
              <span class="block text-[0.6rem] font-bold mt-0.5 opacity-80">&lt; 1 min</span>
            </button>
            <button @click="rateWord(3)" class="py-2 sm:py-3 bg-orange-400 text-black font-black uppercase border-4 border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 hover:shadow-[4px_6px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none transition-all text-xs sm:text-sm">
              Hard
              <span class="block text-[0.6rem] font-bold mt-0.5 opacity-80">6 mins</span>
            </button>
            <button @click="rateWord(4)" class="py-2 sm:py-3 bg-green-400 text-black font-black uppercase border-4 border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 hover:shadow-[4px_6px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none transition-all text-xs sm:text-sm">
              Good
              <span class="block text-[0.6rem] font-bold mt-0.5 opacity-80">1 day</span>
            </button>
            <button @click="rateWord(5)" class="py-2 sm:py-3 bg-blue-400 text-black font-black uppercase border-4 border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5 hover:shadow-[4px_6px_0px_0px_rgba(0,0,0,1)] active:translate-y-0.5 active:shadow-none transition-all text-xs sm:text-sm">
              Easy
      ...
      <div v-else class="absolute inset-0 flex flex-col items-center justify-center px-6">
        ...
        <p class="font-bold text-gray-600 mb-4 text-sm max-w-md text-center">You've reviewed all due cards in this deck for today. Great job staying consistent!</p>
        <button @click="$router.push(`/decks/${deckId}`)" class="px-6 py-3 bg-blue-400 text-black font-black uppercase border-4 border-black rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:bg-blue-500 hover:translate-y-0.5 hover:shadow-none transition-all text-base">
          Back to Deck
        </button>
      </div>
    </div>
